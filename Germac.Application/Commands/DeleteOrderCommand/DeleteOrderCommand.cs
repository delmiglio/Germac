...
{
    public class DeleteOrderCommand(IUnitOfWork unitOfWork, IOrderRepository orderRepository, ILogger logger) : IRequestHandler<DeleteOrderRequest, DeleteOrderResponse>
    {
        ...
        public async Task<DeleteOrderResponse> Handle(DeleteOrderRequest request, CancellationToken cancellationToken)
        {
            try
            {
                logger.Information($"Starting Command {nameof(DeleteOrderCommand)}");
